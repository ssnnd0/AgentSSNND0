...
export async function attach(tabId) {
  ...
  for (const domain of REQUIRED_DOMAINS) {
    await send(tabId, `${domain}.enable`).catch(() => {});
  }
...
chrome.debugger.onEvent.addListener((source, method, params) => {
  ...
  switch (method) {
    case 'Page.javascriptDialogOpening': {
      ...
      send(source.tabId, 'Page.handleJavaScriptDialog', { accept: false }).catch(() => {});
      break;
    }
...
export async function executeAction(tabId, action, elementMap, lastSnapshot) {
  ...
  var center = null;
  if (tool !== 'scroll' && tool !== 'keypress' && tool !== 'done' && tool !== 'navigate') {
  ...
  switch (tool) {
    ...
    case 'scroll': {
      ...
      else expr = 'window.scrollBy({top:' + amt + ',behavior:"instant"});';
      try { await send(tabId, 'Runtime.evaluate', { expression: expr }); } catch {}
      await sleep(300);
